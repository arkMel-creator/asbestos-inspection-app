...
import { Project, User, Sample } from '../types/index';
...
export function Dashboard({ projects, users, currentUser, onNavigate, onOpenProject, onUpdateProject }: DashboardProps) {
  ...
  const myProjects = useMemo(() => {
    return projects.filter(p =>
      p.manager === currentUser.name || (p.assignedStaff || []).includes(currentUser.name)
    );
    ...
    projects.forEach(p => {
      if (p.dueDate) {
        try {
          ...
          console.error('Invalid date', p.dueDate);
        }
      }
    });
    return deadlines;
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden bg-white">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              <div className="flex items-center gap-1.5" style={{ opacity: visibleCategories.jobs ? 1 : 0.4 }}>
                <input type="checkbox" checked={visibleCategories.jobs} onChange={() => setVisibleCategories(v => ({ ...v, jobs: !v.jobs }))} className="w-4 h-4 rounded border-slate-300 cursor-pointer" />
                <span className="px-2 py-1 rounded bg-[#99f6e0] text-[#107569]">Jobs</span>
              </div>
              <div className="flex items-center gap-1.5" style={{ opacity: visibleCategories.quotes ? 1 : 0.4 }}>
                <input type="checkbox" checked={visibleCategories.quotes} onChange={() => setVisibleCategories(v => ({ ...v, quotes: !v.quotes }))} className="w-4 h-4 rounded border-slate-300 cursor-pointer" />
                <span className="px-2 py-1 rounded bg-[#ffd6ae] text-[#bc1b06]">Quotes</span>
              </div>
              <div className="flex items-center gap-1.5" style={{ opacity: visibleCategories.recurring ? 1 : 0.4 }}>
                <input type="checkbox" checked={visibleCategories.recurring} onChange={() => setVisibleCategories(v => ({ ...v, recurring: !v.recurring }))} className="w-4 h-4 rounded border-slate-300 cursor-pointer" />
          ...
          <div className="bg-slate-50 border-b border-slate-200 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <label className="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-white transition-colors">
                <input type="checkbox" checked={visibleCategories.jobs} onChange={() => setVisibleCategories(v => ({ ...v, jobs: !v.jobs }))} className="w-4 h-4 rounded border-slate-300" />
                <span className="text-xs font-medium text-slate-600">Show Project Deadlines</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-white transition-colors">
                <input type="checkbox" checked={visibleCategories.quotes} onChange={() => setVisibleCategories(v => ({ ...v, quotes: !v.quotes }))} className="w-4 h-4 rounded border-slate-300" />
                <span className="text-xs font-medium text-slate-600">Show Quotes</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-white transition-colors">
                <input type="checkbox" checked={visibleCategories.recurring} onChange={() => setVisibleCategories(v => ({ ...v, recurring: !v.recurring }))} className="w-4 h-4 rounded border-slate-300" />
        ...
        <div className="grid grid-cols-7 border-b border-slate-200 bg-white">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((dayName, idx) => {
            ...
            const isTodayDay = currentDay ? isToday(currentDay) : false;
            return (
            ...
            return (
              <div key={idx} className={`relative flex flex-col group ${isTodayDay ? 'bg-slate-50/30' : ''}`}>
                <div className="p-3 text-right">
                  <span className={`text-sm font-semibold ${isTodayDay ? 'text-blue-600 bg-blue-100 rounded-full w-7 h-7 inline-flex items-center justify-center' : 'text-slate-400'}`}>
                    {format(day, 'd')}
                  </span>
                </div>
        ...
        {quickActions.map((action) => (
          ...
          >
            <CardContent className="p-5 flex items-start gap-4">
              <div className={`p-3 rounded-xl border transition-colors ${action.color} group-hover:opacity-80`}>
        ...
        <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 bg-white py-5 px-6">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex overflow-hidden rounded-lg border border-slate-300 divide-x divide-slate-300">
              <div className="px-4 py-2 text-xs font-bold uppercase bg-white text-slate-600">
                Upcoming <span className="ml-2 px-2 py-0.5 rounded-full bg-[#f0f9ff] text-[#026aa2] border border-[#b9e6fe]">0</span>
              </div>
              <div className="px-4 py-2 text-xs font-bold uppercase bg-slate-100 text-slate-600">
                In Progress <span className="ml-2 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">{myProjects.length}</span>
              </div>
              <div className="px-4 py-2 text-xs font-bold uppercase bg-white text-slate-600">
                Overdue <span className="ml-2 px-2 py-0.5 rounded-full bg-red-50 text-red-700 border border-red-200">
                  {myProjects.filter(p => p.dueDate && new Date(p.dueDate) < new Date() && p.status === 'active').length}
                </span>
              </div>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {myProjects.map((project) => (
                ...
                >
                  <td className="px-6 py-4 text-sm font-bold text-emerald-700">{project.jobNumber || '—'}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    <div className="font-medium">{project.name}</div>
                    <div className="text-xs text-slate-400 line-clamp-1">{project.site}</div>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">{project.client}</td>
                  <td className="px-6 py-4">
                    <div
                      className="flex items-center gap-2 group/assign p-1 rounded hover:bg-slate-100 transition-colors w-fit"
              ...
              {myProjects.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-12 text-center text-sm text-slate-400 italic">
                    No projects assigned to you. Go to Projects to create one.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <Card className="w-full max-w-sm shadow-2xl overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between border-b bg-slate-50 py-4">
              <CardTitle className="text-lg">Assign Project Manager</CardTitle>
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => { setAssigningProject(null); setUserSearch(''); }}>
                <X className="h-4 w-4" />
              </Button>
            </CardHeader>
            <CardContent className="p-0">
              ...
              <div className="max-h-[300px] overflow-y-auto">
                ...
                {filteredUsers.length === 0 && (
                  <div className="py-8 text-center text-sm text-slate-400 italic">No users found</div>
                )}
              </div>
            </CardContent>
          </Card>
